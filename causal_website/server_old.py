...
def lamp():
    ...
    # display_object = [o for i, o in enumerate(heat_based_list) if i != rand_indx]
    image_path_list, json_file_list = furnitureloader.load_category(list(["lamp"]));
    # global random_string
    # global encoding
    # random_string = ''.join(random.choice(letters) for i in range(10));
    # encoding = hasher.sha256(random_string.encode('utf-8')).hexdigest();
...
def kerosene_lamp():
    ...
    # display_object = [o for i, o in enumerate(heat_based_list) if i != rand_indx]
    image_path_list, json_file_list = furnitureloader.load_category(list(["kerosene_lamp"]));
    # global random_string
    # global encoding
    # random_string = ''.join(random.choice(letters) for i in range(10));
    # encoding = hasher.sha256(random_string.encode('utf-8')).hexdigest();
...
def candle():
    ...
    # display_object = [o for i, o in enumerate(heat_based_list) if i != rand_indx]
    image_path_list, json_file_list = furnitureloader.load_category(list(["candle"]));
    # global random_string
    # global encoding
    # random_string = ''.join(random.choice(letters) for i in range(10));
    # encoding = hasher.sha256(random_string.encode('utf-8')).hexdigest();
...
def flashlight():
    ...
    # display_object = [o for i, o in enumerate(heat_based_list) if i != rand_indx]
    image_path_list, json_file_list = furnitureloader.load_category(list(["flashlight"]));
    # global random_string
    # global encoding
    # random_string = ''.join(random.choice(letters) for i in range(10));
    # encoding = hasher.sha256(random_string.encode('utf-8')).hexdigest();
...
def wall_lamp():
    ...
    # display_object = [o for i, o in enumerate(heat_based_list) if i != rand_indx]
    image_path_list, json_file_list = furnitureloader.load_category(list(["wall_lamp"]));
    # global random_string
    # global encoding
    # random_string = ''.join(random.choice(letters) for i in range(10));
    # encoding = hasher.sha256(random_string.encode('utf-8')).hexdigest();
...
def oil_lamp():
    ...
    # display_object = [o for i, o in enumerate(heat_based_list) if i != rand_indx]
    image_path_list, json_file_list = furnitureloader.load_category(list(["oil_lamp"]));
    # global random_string
    # global encoding
    # random_string = ''.join(random.choice(letters) for i in range(10));
    # encoding = hasher.sha256(random_string.encode('utf-8')).hexdigest();
...
def receive_plan_data():
    ...
    if step == 1 or step == 2:
        property_path = os.path.join(root, "static/causal_graph/",
                                     "object_property_specific_test_" + obj + "_" + encoding + ".json")
    if step == 3:
        property_path = os.path.join(root, "static/causal_graph/",
                                     "object_property_near_test_" + obj + "_" + encoding + ".json")
    if step == 4:
        property_path = os.path.join(root, "static/causal_graph/",
                                     "object_property_far_test_" + obj + "_" + encoding + ".json")
...
def submit_causal_data():
    ...
    if step == 1:
        # global submit_causal_1
        file_name = "causal_specific_" + display_object[step - 1] + "_" + str(session["submit_causal_1"]) + "_"
    elif step == 2:
        # global submit_causal_2
        file_name = "causal_specific_" + display_object[step - 1] + "_" + str(session.get("submit_causal_2")) + "_"
        # file_name = "causal_specific_"
    elif step == 3:
        # global submit_causal_3
...
def record_time_step1():
    ...
    #     file_name = "causal_far_"
    #     submit_causal_3 = submit_time["time"] - submit_causal_1 - submit_causal_2
    time_path = os.path.join(root, "static/causal_graph/", "submit_time_step1_" + file_name + encoding + ".json")
    with open(time_path, "w") as f:
        if step == 1:
...
def record_time_plan():
    ...
    #     file_name = "causal_far_"
    #     submit_causal_3 = submit_time["time"] - submit_causal_1 - submit_causal_2
    time_path = os.path.join(root, "static/causal_graph/", "submit_time_planning_" + file_name + encoding + ".json")
    with open(time_path, "w") as f:
...
def plan_causal():
    ...
    global display_object
    if step == 1:
        a = website_plan(causal_path, causal_path, encoding, display_object[step - 1],
    ...
    if step == 2:
        ...
        return jsonify(a)
    if step == 3:
        b = website_plan(causal_path, causal_path, encoding, display_object, num=session.get("submit_causal_3"),
                         gen="near")
        print("PRINTING PLAN CAUSAL STUFF ", b)
        return jsonify(b)
        # return jsonify(website_plan(causal_path, causal_path, encoding, display_object, gen="near"))
    elif step == 4:
        c = website_plan(causal_path, causal_path, encoding, display_object, num=0, gen="far")
