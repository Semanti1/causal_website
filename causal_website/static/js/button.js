...
function plan_submit_0(){
  ...
  {
  alert('Fields are empty. Please associate objects to function parts before submitting.Choose \'None of the above\' if no association exists');
  }
...
//   });
   $.ajax({
     ...
     success: function(msg){
       ...
       if($("#text_causal").text()=="successfully saved the causal model."){document.getElementById("planner").disabled = false;}
       else{document.getElementById("planner").disabled = true;}
       if($("#text_causal").text()=="successfully saved the causal model." && document.getElementById("MORESTEP") == null)
{
 document.getElementById("TRANS_BUTT").style.display = "block";
//  document.getElementById("planner").disabled = false;
}
else{document.getElementById("TRANS_BUTT").style.display = "none";d}
//       $("#chksubmit").empty();
//       if (msg[0]=='s')
//      {
//        $("#chksubmit").append(1);
